...
export class ChatTabComponent implements OnInit, AfterViewInit, OnDestroy, OnChanges, AfterViewChecked {
  sampleMessages: ChatMessage[] = [
    this.makeSampleMessage('System', null, 'チュートリアル：はじめに', 'ユドナリウムはサーバーを使用しないTRPGオンセツールです。参加者同士で接続し、コマや画像ファイルなどを同期します。'),
...
任意の場所をクリック : 全ての選択状態を解除`),
    this.makeSampleMessage('System', null, '画像と音楽', 'ファイルをブラウザ画面にドラッグ&ドロップすることでユドナリウムに取り込めます。'),
...
・Chrome ⇔ Firefoxなど違うブラウザを使用する`),
  ...
  get minScrollHeight(): number {
    let length = this.chatTab ? this.chatTab.chatMessages.filter(m => m.isDisplayable).length : this.sampleMessages.length;
  ...
  ngOnInit() {
    EventSystem.register(this)
      ...
      .on('UPDATE_GAME_OBJECT', event => {
        if (this.chatTabList && event.data.identifier === this.chatTabList.identifier) {
          this.changeDetector.markForCheck();
        }
        if (this.chatTab && event.data.identifier === this.chatTab.identifier) {
          this.changeDetector.markForCheck();
        }
      });
  ...
  resetMessages() {
    ...
    // START: リリィ互換 見えないメッセージを考慮したスクロールインデックス再計算
  ...
  private lazyScrollUpdate(isNormalUpdate: boolean = true) {
    ...
    if (chatMessageElements.length === 0) return;
    ...
    if (!isNormalUpdate) this.scrollEventShortTimer.reset();
    if (!isNormalUpdate && !hasTopBlank && !hasBotomBlank) return;
